...
def run():
    ...
    if not threat_data.empty:
        ...
        evento_selecionado = st.selectbox('Selecione um evento de ameaça para editar:', eventos_ameaca)

        # Filtrando os dados para o evento selecionado
        idx = st.session_state.freq_data[st.session_state.freq_data['Evento de ameaça'] == evento_selecionado].index[0]
        row = st.session_state.freq_data.iloc[idx]

        st.write(f"**Editar {row['Evento de ameaça']}**")
